...
// --- Configuration ---
const city = ref('Lyon') // Ville par défaut
...
// const mosqueName = ref('Mosquée locale (exemple)'); // On va remplacer ça
...
const showSearch = ref(false) // Nouvel état pour afficher/masquer la recherche
...
const fetchPrayerTimes = async () => {
  ...
  try {
    ...
    const response = await fetch(
      `https://api.aladhan.com/v1/timingsByCity?city=${city.value}&country=${country.value}&method=${prayerTimeMethod.value}`,
    )
    if (!response.ok) {
...
// --- Gestionnaire d'événement pour la sélection de ville ---
const handleCitySelected = (newCityData) => {
  console.log('Nouvelle ville sélectionnée:', newCityData)
  city.value = newCityData.name
  country.value = newCityData.country // Mettre à jour le pays aussi
  showSearch.value = false // Masquer la recherche après sélection
  fetchPrayerTimes() // Recharger les horaires pour la nouvelle ville
}
...
<template>
  <div class="mawaqit-container container mt-4">
    ...
    <div class="text-center mb-3">
      <button class="btn btn-outline-primary" @click="showSearch = !showSearch">
        <svg
        ...
        {{ showSearch ? 'Fermer la recherche' : 'Changer de ville' }}
      </button>
    </div>

    <!-- Composant de Recherche (conditionnellement affiché) -->
    <SearchScreen
      v-if="showSearch"
      :current-country="country"
      @city-selected="handleCitySelected"
      @search-closed="showSearch = false"
      class="mb-4"
    />
    ...
    <div class="card shadow-sm prayer-card">
      ...
      <div class="card-body">
        <!-- Affichage Erreur -->
        <div v-if="error" class="alert alert-danger">Erreur: {{ error }}</div>
...
@media (max-width: 576px) {
  ...
  /* Style global pour l'app wrapper si besoin */
  .mawaqit-app-wrapper {
    padding-bottom: 2rem; /* Espace en bas */
  }

  /* Ajustement pour que le conteneur principal des horaires ne soit pas trop écrasé par la recherche */
  .mawaqit-container {
    max-width: 600px;
    margin-left: auto;
    margin-right: auto;
